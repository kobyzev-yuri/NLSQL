...
class EnhancedQATrainer:
    ...
    async def _train_with_performance_context(self, question: str, sql: str, context: str):
        ...
        try:
            ...
            # с передачей enhanced_context
            logger.info("✅ Контекст производительности добавлен к обучению")
            
        except Exception as e:
            logger.error(f"❌ Ошибка обучения с контекстом: {e}")
            raise
