...
**Функциональность**:
- **Статистика комментариев**: Показывает общее количество таблиц/колонок, сколько с комментариями, процент покрытия
- **Поиск таблиц**: Поиск по названию для быстрого нахождения нужной таблицы
- **Фильтрация**: Показ всех таблиц, только с комментариями или только без комментариев
- **Комментирование таблиц**: Добавление и редактирование комментариев к таблицам прямо в интерфейсе
- **Комментирование колонок**: Опциональная загрузка колонок для комментирования (только для нужных таблиц)

**Использование**:
1. Перейдите на вкладку "📝 Документирование БД"
2. Используйте поиск для фильтрации таблиц
3. Раскройте нужную таблицу для добавления/редактирования комментария
4. Для комментирования колонок нажмите "📋 Показать колонки для комментирования" внутри таблицы

**API эндпоинты**:
- `GET /api/database/tables` - Список таблиц с комментариями
- `GET /api/database/tables/{table_name}/columns` - Колонки таблицы с комментариями
- `POST /api/database/tables/{table_name}/comment` - Добавить/обновить COMMENT ON TABLE
- `POST /api/database/tables/{table_name}/columns/{column_name}/comment` - Добавить/обновить COMMENT ON COLUMN
- `GET /api/database/comments/stats` - Статистика по комментариям
